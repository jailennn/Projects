...
# Apply 20% discount if both a K race and a marathon are selected
if [ -n "$selected_k_race" ] && [ -n "$selected_marathon" ]; then
    discount=$(echo "scale=2; $total_price * 0.20" | bc)
    total_price=$(echo "scale=2; $total_price - $discount" | bc)
    printf "A 20%% discount has been applied for selecting two races. Discount: \$%.2f\n" "$discount"
fi
...
printf "Total Price after any discounts: \$%.2f\n" "$total_price"
