...
run_trials() {
    ...
    declare -i forward_sequential_count=0
    declare -i backward_sequential_count=0
    ...
    for ((i=0; i<$trials; i++)); do
        ...
        if (( num_dice == 2 )); then
            ...
            if (( i >= 2 )); then
                ...
                # Check for forward sequential rolls
                if [[ "$rolled_numbers" -eq "$((prev_roll_1 + 1))" && "$rolled_numbers" -eq "$((prev_roll_2 + 2))" ]]; then
                    ((forward_sequential_count++))
                fi

                # Check for backward sequential rolls
                if [[ "$rolled_numbers" -eq "$((prev_roll_1 - 1))" && "$rolled_numbers" -eq "$((prev_roll_2 - 2))" ]]; then
                    ((backward_sequential_count++))
                fi
    ...
    if (( num_dice == 1 )); then
        echo "Forward Sequential Rolls - $forward_sequential_count"
        echo "Backward Sequential Rolls - $backward_sequential_count"
    fi
}
